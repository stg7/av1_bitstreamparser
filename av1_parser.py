...
def parse_video(video_file):
    """ parse a av1 video file and extract qp values and other stats """
    ...
    for l in res:
        if "qp=" in l:
            # its a qp=.. line
            qp_values.append(int(l.split("=")[1].strip()))
        else:
            remaining.append(l)
    ...
    per_frame = []
    qp_values = qp_values[0:-1]  # i dont know why there is one more qp value, we just ignore the last value
    # print(qp_values[0],qp_values[1],len(qp_values), len(pkt_sizes), len(key_frames), len(pict_types))

    for i, qp in enumerate(qp_values):
        per_frame.append({
            "qp": qp,
            "pkt_size": pkt_sizes[i],
            # "key_frame": key_frames[i],  # this also seems always = 1
            # "pict_type": pict_types[i]  # it seems that pict_types are not supported by av1, there were only ?
        })
    return {
        "per_frame": per_frame,
...
def main(_):
    # argument parsing
    parser = argparse.ArgumentParser(description='extract qp values from av1 encoded videos',
                                     epilog="stg7 2019",
